...
class Helpers
{
  ...
  {
    $query = Product::whereHas('store', function (Builder $stores) use ($store_slug) {
      $stores->where('slug', $store_slug);
      
      // Apply store approval filtering for non-admin users
      if (self::isUserLogin()) {
        ...
        // For guest users, apply store approval filtering
        $settings = self::getSettings();
        $storeAutoApprove = $settings['activation']['store_auto_approve'] ?? true;
        if (!$storeAutoApprove) {
          $stores->where('is_approved', true);
  ...
  {
    $query = Product::whereNull('deleted_at')?->whereIn('id', $ids);
    ...
    if (self::isUserLogin()) {
      ...
      if ($roleName !== RoleEnum::ADMIN) {
        ...
        $storeAutoApprove = $settings['activation']['store_auto_approve'] ?? true;
        if (!$storeAutoApprove) {
          $query = $query->whereHas('store', function (Builder $stores) {
            $stores->where('is_approved', true);
          });
        }
      }
